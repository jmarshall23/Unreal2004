...
UTVConnection::UTVConnection( SOCKET InSocket, UNetDriver* InDriver, sockaddr_in InRemoteAddr, EConnectionState InState, UBOOL InOpenedLocally, const FURL& InURL )
:	UNetConnection	( InDriver, InURL )
,	Socket			( InSocket )
,	RemoteAddr		( InRemoteAddr )
,	OpenedLocally	( InOpenedLocally )
{
	guard(UTVConnection::UTVConnection);
...
void UTVConnection::LowLevelSend( void* Data, INT Count )
{
	guard(UTVConnection::LowLevelSend);
	if( ResolveInfo )
	{
...
FString UTVConnection::LowLevelGetRemoteAddress()
{
	guard(UTVConnection::LowLevelGetRemoteAddress);
	return IpString(RemoteAddr.sin_addr,ntohs(RemoteAddr.sin_port));
	unguard;
}

FString UTVConnection::LowLevelDescribe()
{
	guard(UTVConnection::LowLevelDescribe);
...
//
UBOOL UTVNetDriver::InitConnect( FNetworkNotify* InNotify, FURL& ConnectURL, FString& Error )
{
	guard(UTVNetDriver::InitConnect);
	...
	ServerConnection = new UTVConnection( Socket, this, TempAddr, USOCK_Pending, 1, ConnectURL );
...
UBOOL UTVNetDriver::InitListen( FNetworkNotify* InNotify, FURL& LocalURL, FString& Error )
{
	guard(UTVNetDriver::InitListen);
...
void UTVNetDriver::TickDispatch( FLOAT DeltaTime )
{
	guard(UTVNetDriver::TickDispatch);
	...
	for( ; ; )
	{
		...
		for( INT i=0; i<ClientConnections.Num() && !Connection; i++ )
			if( IpMatches( ((UTVConnection*)ClientConnections(i))->RemoteAddr, FromAddr ) )
				Connection = (UTVConnection*)ClientConnections(i);
		...
		else
		{
			...
			{
				Connection = new UTVConnection( Socket, this, FromAddr, USOCK_Open, 0, FURL() );
...
FString UTVNetDriver::LowLevelGetNetworkNumber()
{
	guard(UTVNetDriver::LowLevelGetNetworkNumber);
	return IpString(LocalAddr.sin_addr);
	unguard;
}

void UTVNetDriver::LowLevelDestroy()
{
	guard(UTVNetDriver::LowLevelDestroy);
...
// UTVNetDriver interface.
UBOOL UTVNetDriver::InitBase( UBOOL Connect, FNetworkNotify* InNotify, FURL& URL, FString& Error )
{
	guard(UTVNetDriver::UTVNetDriver);
...
UTVConnection* UTVNetDriver::GetServerConnection() 
{
	return (UTVConnection*)ServerConnection;
}

//
// Return the NetDriver's socket.  For master server NAT socket opening purposes.
//
FSocketData UTVNetDriver::GetSocketData()
{
	FSocketData Result;
...
void UTVNetDriver::StaticConstructor()
{
	guard(UTVNetDriver::StaticConstructor);
