...
S32 L3_init(void)
{
   ...
   for (i=0; i < 17; i++)
      ...
      for (j=0; j < 32; j++)
         {
         t_dewindow[i][j] = const_t_dewindow[i][j] * 16383.5F;
         }
      }
   ...
   for (S32 tbl=0; tbl < 34; tbl++)
      ...
      h_tree[tbl] = (HUFFNODE *)
         malloc(MAX_HNODES * sizeof(HUFFNODE));
      ...
      HUFFNODE *N = h_tree[tbl];

      memset(N, 0, MAX_HNODES * sizeof(HUFFNODE));
      ...
      for (;src->x != 100; src++)
         ...
         for (S32 i=0; i < src->len; i++)
            ...
            S32 *dir = &N[cur].child[src->string[i] - '0'];
...
//         assert((!(N[cur].child[0] || N[cur].child[1])));
   ...
   power_fn = (F32 *) malloc(8192 * sizeof(F32));
   ...
   for (i=0; i < 256; i++)
      {
      lfac[i] = powf(2.0F,-0.5F * F32(i));
      sfac[i] = powf(2.0F,-2.0F * F32(i));
      }
...
/*   S32 i,j,k;
   ...
   const S16 *t_l = sf_bound_l[STR->MPEG1][STR->sampling_frequency];
   const S16 *t_s = sf_bound_s[STR->MPEG1][STR->sampling_frequency];
                               ...
                               F32 k[2][576])
{
   if (is_pos == 0)
      ...
      {
      k[0][i] = powf(io,(F32) ((is_pos + 1) / 2));
      k[1][i] = 1;
      }
   else
      {
      k[0][i] = 1;
      k[1][i] = powf(io,(F32) (is_pos / 2));
      }
}
                  ...
                  for (; sb > 0; sb--)
                     ...
                     if (is_pos[i] != 7)
                        ...
                        if (!STR->MPEG1)
                           ...
                           {
                           is_ratio[i] = tanf(is_pos[i] * (PI / 12.0f));
                           }
                        }
...
#if ISO_DCT
   const S16   *xchg = reord_ft[STR->sampling_frequency];
#else
   const S16   *xchg = reord_ft[STR->MPEG1][STR->sampling_frequency];
#endif
...
S32 L3_frame_info(ASISTREAM  *STR)
{
   ...
   S32 result = fread( &STR->header_buffer[STR->header_size], 1, STR->side_info_size, STR->file_handle );

   if (result != STR->side_info_size)
      {
      strcpy(ASI_error_text,"Invalid MPEG audio stream");
      return 0;
      }
