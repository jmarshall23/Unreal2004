...
public:
    ...
    {
		guard(FFileManagerAnsi::IsReadOnly);
...
#endif
		{
			...
			if( hFind != -1 )
			{
				do
				{
                    ...
                    winToUNICODE( Name, Found.name, Min<INT>( ARRAY_COUNT(Name), winGetSizeUNICODE(Found.name) ) );
	...
	{
		guard(FFileManagerAnsi::CompareFileTimes);
