...
public:
	// Get bytes.
	BYTE* PushBytes( size_t AllocSize, INT _Align )
	{
		// Debug checks.
		guardSlow(FMemStack::PushBytes);
		checkSlow(AllocSize>=0);
		checkSlow((_Align&(_Align-1))==0);
		checkSlow(Top<=End);

        PTRINT Align = (PTRINT) _Align;
...
// !!! FIXME: Don't cast to DWORD! Fix appMemzero()'s declaration!  --ryan.
	appMemzero( Result, (DWORD) Size*Count );
	return Result;
	unguardSlow;
}
inline void* operator new( size_t Size, FMemStack& Mem, EMemOned Tag, INT Count, INT Align )
{
	...
	BYTE* Result = Mem.PushBytes( Size*Count, Align );
// !!! FIXME: Don't cast to DWORD! Fix appMemset()'s declaration! --ryan.
	appMemset( Result, 0xff, (DWORD) Size*Count );
	return Result;
	unguardSlow;
}
