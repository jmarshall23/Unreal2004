...
#include "McdDtBridge.h"    /* Cd to Dynamics Bridge: prefix McdDtBridge      */
...
McdDtBridgeID bridge; /* Connection between dynamics and collision */
int geoTypeCount;
  ...
  while( overflow )
    {
      ...
      McdDtBridgeGenerateContacts( bridge, &pairs );
...
void stepEvolve()
{
  ...
  if ( autoBenchmark ) {
    ...
    if(!stopped) MdtWorldStep(world,step);
  ...
  /*fprintf(stderr,"contacts: %d\n:",McdDtBridgeGetUsedContactsCount(bridge));*/
}
...
/*  (MeSqrt(MeSqr((ixGrid/2-ix)/ixGrid)+MeSqr((iyGrid/2-iy))/iyGrid)); */
  ...
  material = McdDtBridgeGetDefaultMaterialID();
  props = McdDtBridgeGetContactParams(material,material);
  ...
  McdDtBridgeSetBody(bridge, convCM, conv);
  ...
  McdDtBridgeSetBody(bridge, gridCM, 0 );
  ...
  /*
  iDefaultRequestID = McdIntersectGetDefaultRequestID();

  iCylReqID = McdIntersectGetNewRequestID();
  McdIntersectSetRequestID( cyl1CM, iCylReqID);

  McdIntersectRequestSetContactMaxCount( iDefaultRequestID, iDefaultRequestID, 4);
  McdIntersectRequestSetContactMaxCount( iDefaultRequestID, iCylReqID, 4);
  */
...
/*  McdDtBridgeSetContactCB(material, material, McduCollectMcdContacts); */
  McdDtBridgeSetActiveTouchCB(material, material, McduCollectContactPairs);
