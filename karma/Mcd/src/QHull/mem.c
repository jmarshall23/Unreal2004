...
static int MEAPI_CDECL qh_intcompare(const void *i, const void *j);
...
void *qh_memalloc(int insize) {
  ...
  if ((unsigned) insize <= (unsigned) qhmem.LASTsize) {
	...
	if (!(newbuffer= MeMemoryAPI.create(bufsize))) {
	...
	*((void **)newbuffer)= qhmem.curbuffer;  /* prepend newbuffer to curbuffer 
    ...
    if (!(object= MeMemoryAPI.create(outsize))) {
      fprintf(qhmem.ferr, "qhull error (qh_memalloc): insufficient memory\n");
      qh_errexit(qhmem_ERRmem, NULL, NULL);
    }
...
void qh_memfreeshort (int *curlong, int *totlong) {
  ...
  *totlong= qhmem .totlong;
  for(buffer= qhmem.curbuffer; buffer; buffer= nextbuffer) {
    ...
    MeMemoryAPI.destroy(buffer);
  }
  qhmem.curbuffer= NULL;
  if (qhmem .LASTsize) {
    MeMemoryAPI.destroy (qhmem .indextable);
    MeMemoryAPI.destroy (qhmem .freelists);
    MeMemoryAPI.destroy (qhmem .sizetable);
  }
...
void qh_meminitbuffers (int tracelevel, int alignment, int numsizes, int bufsize, int bufinit) {
  ...
  qhmem.sizetable= (int *) MeMemoryAPI.createZeroed (numsizes * sizeof(int));
  qhmem.freelists= (void **) MeMemoryAPI.createZeroed (numsizes * sizeof(void *));
...
void qh_memsetup (void) {
  ...
  if (!(qhmem.indextable= (int *)MeMemoryAPI.create((qhmem.LASTsize+1) * sizeof(int)))) {
...
void *qh_memalloc(int insize) {
  ...
  if (!(object= MeMemoryAPI.create(insize))) {
    fprintf(qhmem.ferr, "qhull error (qh_memalloc): insufficient memory\n");
    qh_errexit(qhmem_ERRmem, NULL, NULL);
  }
  if (qhmem.IStracing >= 5)
