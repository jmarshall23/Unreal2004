...
void qh_freebuffers (void) {
  ...
  if (qh temp_malloc)
    MeMemoryAPI.destroy (qh temp_malloc);
  if (qh feasible_point)      /* allocated by qh_readfeasible */
    MeMemoryAPI.destroy (qh feasible_point);
  if (qh feasible_string)     /* allocated by qh_initflags */
    MeMemoryAPI.destroy (qh feasible_string);
  qh line= qh feasible_string= NULL;
  qh half_space= qh feasible_point= qh temp_malloc= NULL;
  /* usually allocated by qh_readinput */
  if (qh first_point && qh POINTSmalloc) {
    MeMemoryAPI.destroy(qh first_point);
    qh first_point= NULL;
  }
  if (qh input_points && qh input_malloc) { /* set by qh_joggleinput */
    MeMemoryAPI.destroy (qh input_points);
...
void qh_initflags(char *command) {
  ...
  while (*s) {
	...
	qh_strtod (s, &t);
      ...
      if (start < t) {
        if (!(qh feasible_string= (char*)MeMemoryAPI.createZeroed (t-start+1))) {
...
#ifdef NGC
	    s++;
	    fprintf (qh ferr, "qhull warning: option 'TO' ignored on GameCube.\n");
#else
...
/*
  timedata is never used after time (&timedata) is called, so we assume we
  don't care about its contents in this use case! - wfgg
*/
  time_t timedata;
...
#if qh_QHpointer
  if (!(qh_qh= (qhT *)MeMemoryAPI.create (sizeof(qhT)))) {
