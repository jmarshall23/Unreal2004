...
static int MEAPI_CDECL qh_compare_facetarea(const void *p1, const void *p2);
static int MEAPI_CDECL qh_compare_facetmerge(const void *p1, const void *p2);
static int MEAPI_CDECL qh_compare_facetvisit(const void *p1, const void *p2);
       int MEAPI_CDECL qh_compare_vertexpoint(const void *p1, const void *p2); /* not used */
...
*/
int MEAPI_CDECL qh_compare_vertexpoint(const void *p1, const void *p2) {
...
int qh_readfeasible (int dim, char *remainder) {
  ...
  if (!(qh feasible_point= (coordT*)MeMemoryAPI.create(dim* sizeof(coordT)))) {
...
coordT *qh_readpoints(int *numpoints, int *dimension, boolT *ismalloc) {
  ...
  if (qh HALFspace) {
    qh half_space= coordp= (coordT*)MeMemoryAPI.create (qh normal_size + sizeof(coordT));
    if (qh CDDinput) {
  ...
  qh line= (char*)MeMemoryAPI.create ((qh maxline+1) * sizeof (char));
  *ismalloc= True;  /* use malloc since memory not setup */
  coords= points= qh temp_malloc=
        (coordT*)MeMemoryAPI.create((*numpoints)*(*dimension)*sizeof(coordT));
...
Type 'qhull' for a short list of options.\n");
  ...
  MeMemoryAPI.destroy (qh line);
  qh line= NULL;
  if (qh half_space) {
    MeMemoryAPI.destroy (qh half_space);
    qh half_space= NULL;
  }
  qh temp_malloc= NULL;
...
Either prepend the input with 1 point or use 'Hn,n,n'.  See manual.\n");
  ...
  if (!(qh feasible_point= (pointT*)MeMemoryAPI.create (dim* sizeof(coordT)))) {
