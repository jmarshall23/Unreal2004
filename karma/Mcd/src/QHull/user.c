...
int qh_fprintf(FILE *f,const char *fmt, ...)
{
    va_list ap;
...
int qh_new_qhull (int dim, int numpoints, coordT *points, boolT ismalloc, 
  ...
  boolT new_ismalloc;
  /* having this static in causes QHull to always return a null hull when an invalid point set is
     passed in (ie a plane), even though subsequent point sets are ok. What was the static doing
     there in the first place??.
     James French, 15/10/01.
  */
