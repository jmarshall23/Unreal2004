...
libraries may need to be added for a Win32 project:
...
The following libraries may be needed for an Xbox project:
...
All the source code of this release, including the demos and
techdemos, can be built in any UNIX-like environment with a Bourne shell
...
recognized are "ps2", "win32", "xbox" and "linux". Run the following command to
...
Similarly you can build only the demos or techdemos or a single
component by going to that subdirectory and executing the following
commands (e.g. for techdemos):
...
Resulting demo and tech demo executables are put in 'demos/bin.rel'
and 'techdemos/bin.rel' respectively for release versions.

Debug versions will be generated into 'demos/bin.dbg' and
'techdemos/bin.dbg'.

Check_release builds will generate files into 'demos/bin.chk' and
'techdemos/bin.chk', respectively.
...
Cygwin may be downloaded as for Win32 above.  Microsoft Visual Studio,
and additionally the Microsoft Xbox SDK are required to build from within
the Cygnus CYGWIN environment.

An additional step is required in building the demos and technical demos.
When 'make' has built the .exe files Xbox images (.xbe) must be built using
'imagebld' which is distributed with the Microsoft Xbox SDK.  We recommend
using the imagebld flags defined in the file MeXbImg.flags, found in the 
src/ directory.  A script, build_xbe.sh, is supplied which will build the
.xbe files. It is located in the src/ directory and should be run from there.

When copying the Xbox images to the Xbox they must be placed in the directory
containing the Resources directory.
 
Linux
-----

MathEngine currently supports the RedHat version 6 or later Linux
distributions.  To build and run the demos, you need an OpenGL
...
'README_ps2libs.txt' in the 'metoolkit' folder for the PlayStation
2 version of Karma.
...
To build using the SNSys compiler on Playstation 2
-----------------------------------------------

Option 1: Use Make
------------------

If you have Cygwin installed, you should be able to build the demos
programs using the supplied makefiles.  For example, the makefile for
the Ballman example can be found in 'metoolkit\src\demos\BallMan'.
...
When building the SNSys compiler will possibly output a number of warnings.
...
Also see the "README_PS2_PRODG.txt" file.

To build using the Codewarrior compiler on Playstation 2
--------------------------------------------------------
Project files to build the Karma Libraries, Demos and TechDemos are
...
'metoolkit\src\TechDemos\TechDemos.mcp'

See the "README_PS2_CWPS2.txt" file for more information.
