...
/*
   ...
   Date: $Date: 2002/11/04 17:22:38 $ - Revision: $Revision: 1.24.2.2.4.1 $
 ...
 * ------------------------------------
 *
 * This function holds all the code that loops over bodies that is not the
 * integrator
 *
 * On Entry:
 *
 * invIworld        - an uninitialised array of size 12*num_bodies MeReals
 * tlist            - an array of 4*4 matrices in column major order. Array is
 *                    of length num_bodies
 * blist[i].invmass - inverse mass of body i (for all i, 0<=i<num_bodies)
 * blist[i].invI0   - a 3*4 symmetric matrix. element 1,4 is the inverse mass.
 *                    elements 2,4 and 3,4 are uninitialised
 *                    (for all i, 0<=i<num_bodies )
 * blist[i].I0      - a 3*4 symmetric matrix. elements 1,4 2,4 and 3,4 are
 *                    uninitialised (for all i, 0<=i<num_bodies )
 * blist[i].vel     - a 4 element vector, first 3 elements are the velocity of
 *                    body i, 4th is uninitialised (for all i, 0<=i<num_bodies)
 * blist[i].velrot  - a 4 element vector, first 3 elements are the rotational
 *                    velocity of body i, 4th is uninitialised
 *                    (for all i, 0<=i<num_bodies )
 * blist[i].force   - a 4 element vector, first 3 elements are the force of
 *                    body i, 4th is uninitialised
 *                    (for all i, 0<=i<num_bodies )
 * blist[i].torque  - a 4 element vector, first 3 elements are the torque of
 *                    body i, 4th is uninitialised
 *                    (for all i, 0<=i<num_bodies )
 ...
 * On Exit:
 *
 * Iworld[i]        = transpose(tlist[i])*matrix_part(blist[i].I0)*tlist[i]
 *                    (for all i, 0<=i<num_bodies )
 * invIworld[i]     = transpose(tlist[i])*matrix_part(blist[i].invI0)*tlist[i]
 *                    (for all i, 0<=i<num_bodies )
 *                    element 1,4 of invIworld[i] = blist[i].invmass
 * blist[i].torque  = blist[i].torque-cross(blist[i].velrot[i],
 *                    matrix_part(Iworld[i])*velrot) 
 *                    (for all i, 0<=i<num_bodies ),
 *                    if blist[i].flags & MdtKeaBodyFlagIsConstrained
 *                  = blist[i].torque, otherwise
 *
               ...
               MeReal                      stepsize   /* Input */)
{
    ...
    {        
        ...
        if((*pb)->flags&MdtKeaBodyFlagAddCoriolisForce)
        {
            ...
            (*pb)->torque[0]+=coriolis[0];
            (*pb)->torque[1]+=coriolis[1];
            (*pb)->torque[2]+=coriolis[2];
    ...
    for(body=0;body!=num_bodies;body++)
    {
        for(i=0;i!=3;i++) pvhmf[i] = hinv*blist[body]->vel[i] + blist[body]->force[i]*invMworld->invmass;
        pvhmf[3]=0.0f;

        for(i=0;i!=3;i++) pvhmf[i+4] = hinv*blist[body]->vel[i+4];
        for(i=0;i!=3;i++) pvhmf[i+4] = pvhmf[i+4] + blist[body]->torque[0] * invMworld->invI0[i];
        for(i=0;i!=3;i++) pvhmf[i+4] = pvhmf[i+4] + blist[body]->torque[1] * invMworld->invI1[i];
        for(i=0;i!=3;i++) pvhmf[i+4] = pvhmf[i+4] + blist[body]->torque[2] * invMworld->invI2[i];
        pvhmf[7]=0.0f;
