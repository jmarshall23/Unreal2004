...
class DLL_EXPORT_CLASS UWaterTexture : public UFractalTexture
{
	...
	// Persistent variables:
	BYTE		DropType GCC_PACK(4);       // Warning: force all 'enum' types to be BYTEs.	
    BYTE		WaveAmp; 
	// FX specifics
	BYTE		FX_Frequency;
	BYTE		FX_Phase;
...
class DLL_EXPORT_CLASS UFluidTexture : public UWaterTexture
{
...
class DLL_EXPORT_CLASS UIceTexture : public UFractalTexture
{
	...
	BYTE        Frequency;
	BYTE        Amplitude;
	BITFIELD    MoveIce:1 GCC_PACK(4);// Mirrored as BOOLEAN in C++
