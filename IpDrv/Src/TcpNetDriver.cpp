...
void UTcpNetDriver::TickDispatch( FLOAT DeltaTime )
{
	...
	for( ; ; )
	{
		...
		else
		{
			...
			INT SameIPCount=0;
			for( INT i=0; i<ClientConnections.Num() && !Connection; i++ )
				if( IpAddressMatches( ((UTcpipConnection*)ClientConnections(i))->RemoteAddr, FromAddr ) && ((UTcpipConnection*)ClientConnections(i))->OpenedTime > (GCurrentTime - 60.0) )
					SameIPCount++;
