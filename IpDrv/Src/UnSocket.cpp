...
TCHAR* SocketHostError( INT Code )
{
#if __WINSOCK__
	return(SocketError(Code));
#elif __BSD_SOCKETS__
	// True BSD systems (like OS X) use "h_errno" instead of
	//  "errno" for gethostbyname() failures, so we need to
	//  catch this here.  --ryan.
#if __BSD_SOCKETS_TRUE_BSD__
	if (Code == -1)
		Code = h_errno;  // This is "errno" on Linux.
#endif
...
#endif
{
	...
	if( HostEnt==NULL || HostEnt->h_addrtype!=PF_INET )
		appSprintf( Info->Error, TEXT("Can't find host %s (%s)"), appFromAnsi(Info->HostName), SocketHostError(e) );
	else
...
UBOOL InitSockets( FString& Error )
{
	...
	if( Mismatch )
		appErrorf(TEXT("IpDrv C++/UnrealScript class size mismatch"));
	...
	if( !Tried )
	{
		...
		xnsp.cfgPrivatePoolSizeInPages		= 24; //12
		xnsp.cfgEnetReceiveQueueLength		= 8;
		xnsp.cfgIpFragMaxSimultaneous		= 4;
		xnsp.cfgSockMaxSockets				= 64;
		xnsp.cfgSockDefaultRecvBufsizeInK	= 255; //16
		xnsp.cfgSockDefaultSendBufsizeInK	= 255; //16
		xnsp.cfgKeyRegMax					= 4;
		xnsp.cfgSecRegMax					= 4;
		xnsp.cfgQosDataLimitDiv4			= 128;
...
#if _MSC_VER
	return setsockopt( Socket, SOL_SOCKET, SO_REUSEADDR, (char *)&optval, sizeof(INT) ) == 0;
#else
	return setsockopt( Socket, SOL_SOCKET, SO_REUSEADDR, &optval, sizeof(INT) ) == 0;
#endif
}
...
#ifdef LINUX
	int on = 1;
	return setsockopt(Socket, SOL_IP, IP_RECVERR, &on, sizeof (on)) == 0;
#else
	return 1;
#endif
}
