...
// Types.
typedef void (USetupDefinition::*INSTALL_STEP)(FString,FString,UBOOL,INT,FInstallPoll*);
...
struct FFileInfo
{
	...
	SQWORD  Size, RefSize, CompSize;
	UBOOL   MasterRecurse, Compressed, Optional;
	DWORD   Flags;
	INT		CDNum;
	FFileInfo( FString Init )
	: Dest(), Src(), Ref(), Lang(), Size(0), RefSize(0), MasterRecurse(0), Flags(0), CDNum(0), Compressed(0), CompSize(0), Optional(0)
	{
		...
		Parse( *Init, TEXT("COMPSIZE="),CompSize);
		Parse( *Init, TEXT("SIZE="),   Size   );
		Parse( *Init, TEXT("LANG="),   Lang   );
		Parse( *Init, TEXT("FLAGS="),  Flags  );
		Parse( *Init, TEXT("CD="),     CDNum  );
		ParseUBOOL( *Init, TEXT("MASTERRECURSE="), MasterRecurse );
		ParseUBOOL( *Init, TEXT("COMPRESSED="), Compressed );
		ParseUBOOL( *Init, TEXT("OPTIONAL="), Optional );
		unguard;
	}
	...
	void Write( FOutputDevice& Ar, UBOOL Distribution )
	{
		...
		if( CompSize!=0 )
			Ar.Logf( TEXT(",CompSize=%i"), CompSize );
		if( Flags!=0 )
			Ar.Logf( TEXT(",Flags=%i"), Flags );
		if( CDNum!=0)
			Ar.Logf( TEXT(",CD=%i"), CDNum );
		if( Distribution && MasterRecurse )
			Ar.Logf( TEXT(",MasterRecurse=True") );
		if( Compressed )
			Ar.Logf( TEXT(",Compressed=True") );
		if( Optional )
			Ar.Logf( TEXT(",Optional=True") );
		Ar.Log( TEXT(")") );
		unguard;
	}
};
...
{
	DECLARE_ABSTRACT_CLASS(USetupObject,UObject,0,Setup)
};
...
{
	DECLARE_CLASS(USetupProduct,USetupObject,CLASS_Config|CLASS_Localized|CLASS_PerObjectConfig,Setup)
...
{
	DECLARE_CLASS(USetupGroup,USetupObject,CLASS_Config|CLASS_Localized|CLASS_PerObjectConfig,Setup)
...
{
	DECLARE_CLASS(USetupDefinition,USetupProduct,CLASS_Config|CLASS_Localized|CLASS_PerObjectConfig,Setup)
	...
	UBOOL					NoRun;
	UBOOL					Manifest;
	UBOOL					HardDiskCDInstall;	// multiple CDs are copied to a hard disk tree.
	UBOOL					InstallCDTempDir;	// We're running a CD install from the temp directory.
	INT						CurrentCDNumber;	// current CD we're on.
	INT						UninstallTotal;
	INT						UninstallCount;
	...
	void StaticConstructor()
	{
		...
		UArrayProperty* P;
		  ...
		  new(GetClass(),TEXT("CheckCDKey"         ),RF_Public)UBoolProperty (CPP_PROPERTY(CheckCDKey         ), TEXT(""), CPF_Config   );
		  new(GetClass(),TEXT("MultiCD"	           ),RF_Public)UBoolProperty (CPP_PROPERTY(MultiCD            ), TEXT(""), CPF_Config   );
		  ...
		  new(GetClass(),TEXT("LastCDNum"          ),RF_Public)UIntProperty  (CPP_PROPERTY(LastCDNum          ), TEXT(""), CPF_Config   );
	...
	USetupDefinition();
	virtual void Init();
 	virtual UBOOL LocateSourceFile( FString& Src, INT CDNum, UBOOL Compressed, UBOOL DontPromptCD );
	virtual UBOOL PromptCD( INT CDNum, const TCHAR* Filename );
	...
	virtual void InstallTree( const TCHAR* Action, FInstallPoll* Poll, void (USetupDefinition::*Process)( FString Key, FString Value, UBOOL Selected, INT GroupCDNum, FInstallPoll* Poll ), USetupGroup* SetupGroup=NULL, UBOOL Selected=1 );
	virtual void UninstallTree( const TCHAR* Action, FInstallPoll* Poll, void (USetupDefinition::*Process)( FString Key, FString Value, INT GroupCDNum, FInstallPoll* Poll ) );
	...
	virtual void PerformInstallCopy();
	...
	// Installation steps.
	virtual void ProcessCheckRef( FString Key, FString Value, UBOOL Selected, INT GroupCDNum, FInstallPoll* Poll );
	virtual void ProcessVerifyCd( FString Key, FString Value, UBOOL Selected, INT GroupCDNum, FInstallPoll* Poll );
	virtual void ProcessPreCopy( FString Key, FString Value, UBOOL Selected, INT GroupCDNum, FInstallPoll* Poll );
	virtual void ProcessCopy( FString Key, FString Value, UBOOL Selected, INT GroupCDNum, FInstallPoll* Poll );
	virtual void ProcessExtra( FString Key, FString Value, UBOOL Selected, INT GroupCDNum, FInstallPoll* Poll );
	virtual void ProcessPostCopy( FString Key, FString Value, UBOOL Selected, INT GroupCDNum, FInstallPoll* Poll );

	// Uninstall steps.
	virtual void ProcessUninstallCountTotal( FString Key, FString Value, INT GroupCDNum, FInstallPoll* Poll );
	virtual void ProcessUninstallRemove( FString Key, FString Value, INT GroupCDNum, FInstallPoll* Poll );
};
