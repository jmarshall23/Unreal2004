...
{
	DECLARE_CLASS(USetupShortcut,USetupObject,CLASS_Config|CLASS_Localized|CLASS_PerObjectConfig,Setup);
...
{
	DECLARE_CLASS(USetupGroupWindows,USetupGroup,CLASS_Config|CLASS_Localized|CLASS_PerObjectConfig,Setup)

	// Variables.
	FString DirectXHook;
	TArray<FString> WinRegistry, WinRegistryNoOverwrite, Shortcut;
	...
	void StaticConstructor()
	{
        ...
        P->Inner = new(P,TEXT("StrProperty0"   ),RF_Public)UStrProperty;
		P=new(GetClass(),TEXT("WinRegistryNoOverwrite"    ),RF_Public)UArrayProperty(CPP_PROPERTY(WinRegistryNoOverwrite   ), TEXT(""), CPF_Config   );
		P->Inner = new(P,TEXT("StrProperty0"   ),RF_Public)UStrProperty;

		unguard;
	}
	USetupGroupWindows()
	: USetupGroup(), WinRegistry(E_NoInit), WinRegistryNoOverwrite(E_NoInit), Shortcut(E_NoInit), DirectXHook(E_NoInit)
	{}
};
...
{
	DECLARE_CLASS(USetupDefinitionWindows,USetupDefinition,CLASS_Config|CLASS_Localized|CLASS_PerObjectConfig,Setup)
	NO_DEFAULT_CONSTRUCTOR(USetupDefinitionWindows);
	...
	class WWindow* OwnerWindow;

	// Functions.
	USetupDefinitionWindows( WWindow* InOwnerWindow );
	UBOOL PromptCD( INT CDNum, const TCHAR* Filename );
	UBOOL GetRegisteredProductFolder( FString Product, FString& Folder );
	void SetupFormatStrings();
	void ProcessPostCopy( FString Key, FString Value, UBOOL Selected, INT GroupCDNum, FInstallPoll* Poll );
	void ProcessExtra( FString Key, FString Value, UBOOL Selected, INT GroupCDNum, FInstallPoll* Poll );
	void PreExit();
	void PerformInstallCopy();
	void ProcessUninstallRemove( FString Key, FString Value, INT GroupCDNum, FInstallPoll* Poll );
...
class WCDPromptDialog : public WDialog
{
	DECLARE_WINDOWCLASS(WCDPromptDialog,WDialog,Setup)

	// Controls.
	WCoolButton OkButton, CancelButton;
	WLabel CDPromptText;
	FString Title;
	FString Message;

	// Constructor.
	WCDPromptDialog( WWindow* InOwnerWindow, USetupDefinition* Manager, const TCHAR* InTitle, const TCHAR* InMessage );
