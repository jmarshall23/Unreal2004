...
local int i;
	...
	Content = Content $ MakeMenuRow(Response, GameType $ "&Page=" $ DefaultsVotingGameConfigPage, DefaultsVotingGameConfigLink);
			...
			switch (Tmp)
			{
					...
					for (i = Count - 1; i >= 0; i--)
					{
						if (ExcludeMaps.Count() > 0)
						{
							...
							if (id >= 0)
							{
								...
								Level.Game.MaplistHandler.AddMap(Index, CurrentList, MapName);
							}
							else
						...
						for (i = Count-1; i >= 0; i--)
						{
							...
							if (IncludeMaps.Count() > 0)
							{
								...
								if (id >= 0)
								{
									...
									Level.Game.MaplistHandler.RemoveMap(Index, CurrentList, MapName);
								}
								else
				...
				case ">>":
					while (ExcludeMaps.Count() > 0)
					{
						...
						{
							MovedMaps.CopyFromId(IncludeMaps, id);
							Level.Game.MaplistHandler.AddMap(Index, CurrentList, IncludeMaps.GetTag(id));
						}
					}
				...
				case "Up":
					...
					for (i = 0; i<IncludeMaps.Count(); i++)
					{
						...
						{
							Level.Game.MaplistHandler.ShiftMap(Index, CurrentList, IncludeMaps.GetTag(i), MoveCount);
...
{
	local int i, j, columns;
	local string PageText, GameConfigData;
	...
	if (CanPerform("Ms"))
	{
		...
		if (Request.GetVariable("Update") != "")
		{
			i = int(Request.GetVariable("GameConfigIndex", "-1"));
			if( i > -1 )
			{
				for( j=0; j < columns; j++ )
				{
					//log("Update " $ i $ ", " $ j $ ", " $ Level.Game.VotingHandler.GetConfigArrayColumnTitle("GameConfig",j) $ " = " $ Request.GetVariable(Level.Game.VotingHandler.GetConfigArrayColumnTitle("GameConfig",j)));
					Level.Game.VotingHandler.UpdateConfigArrayItem("GameConfig", i, j,
					      Request.GetVariable(Level.Game.VotingHandler.GetConfigArrayColumnTitle("GameConfig",j)));
				}
		...
		if(Request.GetVariable("Delete") != "")
		{
			...
			if (i > -1)
			{
				Level.Game.VotingHandler.DeleteConfigArrayItem("GameConfig", i);
				Level.Game.VotingHandler.SaveConfig();
			}
		}
				...
				switch( Caps(Parts[0]) )  // type
				{
					...
					case "MUTATORS":
						PageText $= "<select name=\"" $ Level.Game.VotingHandler.GetConfigArrayColumnTitle("GameConfig",j) $
						            "\" size=2 multiple>" $ GenerateMutatorOptions(Parts[2]) $ "</select>";
