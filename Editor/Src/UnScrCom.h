...
public:
	...
	UBOOL MatchesType( const FPropertyBase& Other, UBOOL Identity ) const
	{
		...
		if( PropertyFlags & CPF_OutParm )
		{
            ...
            if( Type!=CPT_Struct ) // gam --- This allows for derived structs to be passed by reference.
    			Identity = 1;
		}
...
public:
	...
	void AttemptToConvertConstant( const FPropertyBase& NewType )
	{
		...
		switch( NewType.Type )
		{
			...
			case CPT_Pointer:	{PTRINT     V(0);           if( GetConstPointer (V) ) SetConstPointer (V); break;}
	...
	void SetConstPointer( INT InPtr )
	{
		(FPropertyBase&)*this = FPropertyBase(CPT_Pointer);
		Pointer 		= InPtr;
		TokenType		= TOKEN_Const;
	}
	...
	UBOOL GetConstByte( BYTE& B )
	{
		...
		else if( TokenType==TOKEN_Const && Type==CPT_Pointer && Pointer>=0 && Pointer<255)
		{
			B = Pointer;
			return 1;
		}
	...
	UBOOL GetConstInt( INT& I )
	{
		...
		else if( TokenType==TOKEN_Const && Type==CPT_Pointer )
		{
			I = Pointer;
			return 1;
		}
	...
	UBOOL GetConstPointer( PTRINT& P )
	{
		if( TokenType==TOKEN_Const && Type==CPT_Int )
		{
			P = Int;
...
public:
	...
	UBOOL			GetToken( FToken& Token, const FPropertyBase* Hint=NULL, INT NoConsts=0,UBOOL DashHack=0 );
	UBOOL			GetRawToken( FToken& Token );
	void			UngetToken( FToken& Token );
	UBOOL			GetIdentifier( FToken& Token, INT NoConsts=0,UBOOL DashHack=0 );
