...
)
{
	...
	if( iLeaf == INDEX_NONE )
	{
		...
		{
			ASkyZoneInfo*	SkyZoneInfo = Cast<ASkyZoneInfo>(Level->GetZoneActor(Model->Leaves(iLeaf).iZone));

			for(INT LeafIndex = 0;LeafIndex < Model->Leaves.Num();LeafIndex++)
			{
				if(SkyZoneInfo || Model->Leaves(LeafIndex).iZone == Model->Leaves(iLeaf).iZone)
				{
