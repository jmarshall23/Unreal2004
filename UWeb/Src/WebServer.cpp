...
IMPLEMENT_FUNCTION( UWebResponse, INDEX_NONE, execLoadParsedUHTM );
...
IMPLEMENT_FUNCTION( UWebResponse, INDEX_NONE, execGetHTTPExpiration );
/*
void UWebResponse::execDump( FFrame& Stack, RESULT_DECL )
{
	P_FINISH;

	INT i(0);
	for ( TMultiMap<FString,FString>::TIterator It(ReplacementMap); It; ++It )
	{
		FString Text = It.Value();
		if ( Text.Len() > 1000 )
		{
			TArray<FString> Lines;
			while ( Text.Len() > 1000 )
			{
				new(Lines) FString(Text.Left(1000));
				Text = Text.Mid(1001);
			}

			new(Lines) FString(Text);

			debugf(TEXT("%i) %s = %s"), i++, *(It.Key()), *Lines(0));
			for ( INT j = 1; j < Lines.Num(); j++ )
				debugf(TEXT("      %s"), *Lines(j));
		}
		else debugf(TEXT("%i) %s = %s"), i++, *(It.Key()), *(It.Value()));
	}
}
IMPLEMENT_FUNCTION(UWebResponse,INDEX_NONE,execDump);
*/
/*-----------------------------------------------------------------------------
	The End.
-----------------------------------------------------------------------------*/
