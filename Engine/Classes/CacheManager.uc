...
final simulated static function MutatorRecord GetMutatorRecord( coerce string ClassName )
{
	local int i;
	local MutatorRecord empty;

	if ( default.CacheMutators.Length == 0 )
		InitCache();

	for ( i = 0; i < default.CacheMutators.Length; i++ )
		if ( default.CacheMutators[i].ClassName ~= ClassName )
			return default.CacheMutators[i];

	return empty;
}

final simulated static function WeaponRecord GetWeaponRecord( coerce string ClassName )
{
	local int i;
	local WeaponRecord empty;

	if ( default.CacheWeapons.Length == 0 )
		InitCache();

	for ( i = 0; i < default.CacheWeapons.Length; i++ )
		if ( default.CacheWeapons[i].ClassName ~= ClassName )
			return default.CacheWeapons[i];

	return empty;
}

final simulated static function VehicleRecord GetVehicleRecord( coerce string ClassName )
{
	local int i;
	local VehicleRecord empty;

	if ( default.CacheVehicles.Length == 0 )
		InitCache();

	for ( i = 0; i < default.CacheVehicles.Length; i++ )
		if ( default.CacheVehicles[i].ClassName ~= ClassName )
			return default.CacheVehicles[i];

	return empty;
}

final simulated static function AnnouncerRecord GetAnnouncerRecord( coerce string ClassName )
{
	local int i;
	local AnnouncerRecord empty;

	if ( default.CacheAnnouncers.Length == 0 )
		InitCache();

	for ( i = 0; i < default.CacheAnnouncers.Length; i++ )
		if ( default.CacheAnnouncers[i].ClassName ~= ClassName )
			return default.CacheAnnouncers[i];
