...
// Karma Includes
#if !defined(_XBOX) && !defined(__PSX2_EE__) && !defined(__GCN__) && !defined(__LINUX__) && !defined(MACOSX) && !defined (__FreeBSD__) && !defined(NO_KARMA)
#ifndef WITH_KARMA
#  define WITH_KARMA
#endif
#endif
...
// !!! FIXME: x86 gcc generates bad code with 64-bit integers
// !!! FIXME:  (perhaps in conjunction with templates...?) We'll just
// !!! FIXME:  need to wing it with half as many bits. :(  --ryan.
#if __LINUX_X86__
typedef DWORD KModelPairType;
#else
typedef QWORD KModelPairType;
#endif
...
#include "UStream.h"            // Streaming file high-level support.
...
#include "UnCacheManager.h"		// Cache management.
#include "UnAuthenticate.h"		// Content autentication and encryption.
